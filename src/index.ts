...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      // Awaited here, not just returned, so an error thrown inside any route
      // handler lands in this catch instead of escaping it as a raw 500 page.
...
async function route(request: Request, env: Env): Promise<Response> {
  ...
  if (pathname === "/api/suppliers" && method === "POST") {
    if (!role) return error("Not signed in", 401);
    return createSupplier(request, env, role);
  }
  ...
  if (pathname === "/api/suppliers/import" && method === "POST") {
    if (!role) return error("Not signed in", 401);
    return importSuppliers(request, env, role, url.searchParams.get("commit") === "true");
  }
  const supplierMatch = pathname.match(/^\/api\/suppliers\/([^/]+)$/);
  if (supplierMatch && method === "PATCH") {
    // The abbreviation drives Quality's internal batch numbers.
    if (role !== "quality") return error("Only quality can edit supplier details", 403);
    return updateSupplier(request, env, decodeURIComponent(supplierMatch[1]));
  }
  ...
  const importSchemeMatch = pathname.match(/^\/api\/import-code-schemes\/(RMS|RMF|RMP)$/);
  ...
  if (pathname === "/api/test-catalog" && method === "GET") {
    if (role !== "quality") return error("The test catalog is a quality-only view", 403);
    return listTestCatalog(env);
  }
  if (pathname === "/api/test-catalog" && method === "PUT") {
    if (role !== "quality") return error("Only quality can manage the test catalog", 403);
    return upsertTestCatalogEntry(request, env);
  }
  ...
  if (productInfoMatch && method === "PATCH") {
    if (role !== "quality") return error("Only quality can record product details", 403);
    return setLineProductInfo(request, env, Number(productInfoMatch[1]));
  }

  const classificationMatch = pathname.match(/^\/api\/receipt-lines\/(\d+)\/classification$/);
  if (classificationMatch && method === "PATCH") {
    if (role !== "quality") return error("Only quality can change a line's classification", 403);
